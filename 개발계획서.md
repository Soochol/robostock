...
industry_cycle_analysis = {
    ...
    "cycle_stage": "성장기",               # 사이클 단계 (도입/성장/성숙/쇠퇴)
}
```
...
## 2.5 핵심 지표 정의 및 라벨링 전략

### 2.5.1 지지선 체계 (Support Level Hierarchy)

> **핵심 발견**: 성공하는 종목은 특정 기술적 지지선을 지킨다
>
> **중요**: 지지선은 1번 블록 발생 → 2번 블록 발생 **이후**에 테스트됨

#### 지지선 우선순위 (강도 순)

| 우선순위 | 지지선 | 강도 | 예상 수익률 | 발생 빈도 |
|---------|--------|------|------------|----------|
| 1순위 (최강) | 2번 블록 D일 몸통 | ⭐⭐⭐⭐⭐ | 200%+ (Level 4) | ~15% |
| 2순위 (강함) | 1번 블록 D일 몸통 | ⭐⭐⭐⭐ | 100~200% (Level 3) | ~45% |
| 3순위 (약함) | 2번 블록 D/D+1/D+2 저가 | ⭐⭐ | 50~100% (Level 2) | ~25% |
| 실패 | 모든 지지선 붕괴 | ❌ | 손실 (Level 0) | ~15% |

#### 지지선 정의

1. **몸통 (Body)**: 시가와 종가 사이 범위 (캔들의 실체)
   - 양봉: 시가(하단) ~ 종가(상단)
   - 음봉: 종가(하단) ~ 시가(상단)

2. **저가 (Low)**: 당일 최저가 (아래 꼬리 끝)

#### 지지 판정 기준

- **지지 성공**: 지지선 아래 침투 -4% 이내 + 3일 내 회복
- **지지 실패**: 지지선 아래 -5% 이상 침투 또는 3일 연속 이탈
...
```text
2번 거래량 블록 범위의 고점을 3일 연속 종가가 상회
```

#### 구체적 기준

- 기준 가격: 2번 블록 D/D+1/D+2일 중 최고가
- 판정 방법: 3일 연속 종가 > 기준 가격
- 돌파 확정일: 3일 연속 조건을 만족한 마지막 날
...
- 조건: 돌파 확정 + 블록 고점 대비 50% 이상 상승
- Level 분류:
  - **Level 4** (초고수익): 200% 이상 상승
  - **Level 3** (고수익): 100~200% 상승
  - **Level 2** (중수익): 50~100% 상승
  - **Level 1** (저수익): 50% 미만 상승

#### 실패 (돌파 실패 또는 부진)

- 조건 1: 돌파 미확정 (3일 연속 조건 미달)
- 조건 2: 돌파 후 50% 미만 상승
- **Level 0** (실패): 50% 미만 또는 손실

### 2.5.4 실전 매매 기준

#### 진입 전략

| 지지 레벨 | 진입 타이밍 | 기대 수익 | 위험도 |
|----------|-----------|----------|--------|
| 2번 블록 몸통 | 몸통 하단 터치 후 반등 | Level 4 (200%+) | 낮음 |
| 1번 블록 몸통 | 몸통 하단 테스트 후 반등 | Level 3 (100~200%) | 보통 |
| 2번 블록 저가 | 저가 반등 확인 후 | Level 2 (50~100%) | 높음 |

#### 손절 전략

| 경고 단계 | 가격 기준 | 조건 | 조치 |
|----------|----------|------|------|
| 주의 | 지지선 -3% | 1일 이탈 | 모니터링 강화 |
| 경고 | 지지선 -5% | 2일 연속 | 손절 준비 |
| 손절 | 지지선 -5% | 3일 연속 | 즉시 청산 |
...
- **기본 정보**
  ...
  - D, D+1, D+2 각 날짜별 거래대금 및 점수
  - 1번 블록 대비 거래량 비율 (80% 이상)
  - D+1, D+2 고가 돌파 여부 (vs D일 고가)
  - D+1, D+2 조건 충족 여부
- **D, D+1, D+2 당일 수급 데이터 (스냅샷)**
  - 각 날짜별 기관/외국인/외국인+기관/개인/프로그램 순매수 금액
  - 각 주체별 순매수 여부 (Boolean)
  - 각 주체별 매수강세 지수 (%)
- **패턴 매칭**
...
- **블록 범위 데이터 (60일 이동평균선 기준)**
  - 블록 기간 시작/종료 날짜 (2번 D일 ~ 60이평선 회귀일)
  - 블록 기간 (일수)
  - 블록 기간 최고가 (★ 수익률 기준점) 및 달성일
  - 블록 기간 최저가 및 발생일
  - 블록 종료일 OHLC
  - 60이평선 정보 (시작/종료 시점 값, 종료 사유: 터치/하방이탈)
  - 블록 기간 평균 일 거래량
  - 블록 기간 최대 거래량 발생일
  - 비교 지수 (KOSPI/KOSDAQ)
  - **지수 대비 성과**
...
**3번 거래량 블록 데이터**
- **기본 정보**
  - 3번 블록 발생 여부 (has_block_3)
  - 3번 블록 발생일 (d_date)
  - 2번 블록 D일로부터 경과 일수 (6개월 내)
  - 2번 블록 최대 거래량 (비교 기준값)
  - 2번 블록 최고가 (비교 기준값)
  - 거래량 조건: 2번 블록 최대 거래량 대비 15% 이상
  - 가격 조건: 2번 블록 최고가 대비 ±12% 이내
  - 거래량/가격 조건 충족 여부
- **D일 거래량, 가격, 거래대금 데이터**
  - D일 거래량, OHLCV
  - D일 거래대금
  - 2번 블록 최대 거래량 대비 거래량 비율
  - 2번 블록 최고가 대비 가격 차이 (원, %)
- **D일 수급 데이터 (스냅샷)**
  - 기관/외국인/외국인+기관/개인/프로그램 순매수 금액
  - 각 주체별 매수강세 지수
  - 쌍끌이 여부 (외국인+기관 모두 순매수)
- **품질 평가**
  - 품질 점수 (10점 만점)
  - 품질 등급 (S/A/B/C)
- **블록 범위 데이터**
  - 블록 기간 시작/종료 날짜 (D일 ~ 분석 시점/다음 블록 발생)
  - 블록 기간 (일수)
  - 블록 기간 최고가/최저가 및 달성일
  - 평균 일 거래량
  - 비교 지수 (KOSPI/KOSDAQ)
...
- **외국인 투자자**
  - 순매수 금액 및 주식수
  - 보유 비중 변화 (시작/종료 시점 보유 비중)
  - 연속 순매수/순매도 일수
  - 매매 트렌드 강도 (강한 매수/매수/중립/매도/강한 매도)
  - **누적 수급 트렌드**
    - 누적 순매수 (시작/종료)
    - 선형 회귀 기울기 (원/일)
    - 추세 방향 (상승/하락/횡보)
...
- **외국인+기관 합산**
  ...
  - **누적 수급 트렌드**
    - (외국인 투자자와 동일 구조)
- **수급 균형 분석**
  - 외국인/기관/개인 각 추세 (순매수/순매도/중립)
  - 주도 세력 (외국인/기관/개인/외국인+기관)
  - 외국인-기관 동조화 여부
  - 개인 저항 강도 (강함/보통/약함)
  - 수급 균형 점수 (10점 만점)
  - 수급 해석 코멘트
...
- **섹터/테마 주도주 분석**
  - **섹터/테마 분류**
    ...
    - 관련 섹터 지수
  - **상대 강도 분석**
    - 1M/3M/6M 수익률 및 섹터 평균 대비 초과 수익률
    - 섹터 지수 대비 초과 수익률
    - 시장 지수 (KOSPI/KOSDAQ) 대비 초과 수익률
    - 섹터 내 상대 강도 순위 및 백분위
  - **거래량 우위**
    - 일평균 거래대금 및 섹터 평균 대비 배율
    - 섹터 내 거래대금 순위
    - 거래량 증가율 및 섹터 평균 대비 비교
  - **시가총액 및 수급**
    - 시가총액, 섹터 내 순위 및 비중
    - 1번/2번 블록 D일 수급 (기관/외국인 순매수 금액 및 여부)
    - 2번 블록 D+1, D+2일 수급
    - 일평균 회전율, 유동성 순위
    - 고유동성 여부 (일 거래대금 500억 이상)
- **미래 산업 포함 여부**
  - **미래 성장 산업 분류**
    - 미래 성장 산업 여부
    - 해당 카테고리 (AI/전기차/바이오 등, 복수 가능)
  - **기술 혁신성**
    - R&D 투자 비중 (매출 대비 %)
    - R&D 투자액 (연간)
...
- **기준일**: 2번 거래량 블록 범위의 고점
- **추적 기간**: 최대 5년
- **수집 데이터**:

  - **[1순위] 돌파 여부 분석**
    - 돌파 성공 여부 (3일 연속 종가 > 블록 고점)
    - 돌파 확정일 및 소요 일수 (2번 블록 D일 기준)
    - 돌파 시도 횟수 (고점 터치 후 실패 횟수)

  - **[2순위] 지지선 분석** ⭐ NEW
    - **지지선 가격대 (2번 블록 발생 이후 테스트됨)**
      - 2번 블록 D일 몸통 (시가~종가)
      - 1번 블록 D일 몸통 (시가~종가)
      - 2번 블록 D/D+1/D+2일 저가
      - 블록 고점 대비 비율 (%)
    - **실제 지지 레벨**
      - 최종 지지된 레벨 (block_2_body / block_1_body / block_2_lows / broken)
      - 지지 강도 점수 (1~10)
      - 최대 조정폭 (블록 고점 대비 %, 음수)
      - 최저점 발생일 및 소요 일수
    - **지지 테스트 기록**
  ...
  - **[3순위] 상승 폭 분석** (돌파 성공 시)
    - 블록 고점 대비 최대 상승률 (%)
    - 수익률 Level 분류 (0~4)
      - Level 4 (초고수익): 200% 이상 (주로 2번 블록 몸통 지지)
      - Level 3 (고수익): 100~200% (주로 1번 블록 몸통 지지)
      - Level 2 (중수익): 50~100% (주로 2번 블록 저가 지지)
      - Level 1 (저수익): 50% 미만 상승
      - Level 0 (실패): 돌파 실패 또는 손실
    - 최고점 도달까지 소요 일수
    - 6개월, 1년, 2년, 3년, 5년 시점 수익률 (참고용)

  - **[4순위] 실패 원인 분석** (돌파 실패 시)
    - 실패 패턴 분류
      - 지지선_연쇄_붕괴 (모든 지지선 이탈)
      - 수급_약화_조기 (1개월 내 외국인/기관 이탈)
      - 재무_악화 (실적 부진)
      - 섹터_약세 (섹터 전체 하락)
    - 블록 고점 대비 최대 도달률 (%)
    - 결정적 지지선 붕괴 지점 및 날짜
  ...
  - **보조 지표**
    - MDD 분석 (블록 고점 대비 최대 하락률)
    - MDD 발생일 및 소요 일수
    - 리스크-수익 비율 (max_return / |MDD|)
    - 블록 고점 아래 머문 총 일수
    - 주요 가격 변곡점 (고점, 저점, 지지/저항선)
...
- **입력**: 1번 블록 D일 OHLC 데이터
- **알고리즘**:
  1. 몸통 상단 = max(시가, 종가)
  2. 몸통 하단 = min(시가, 종가)
  3. 양봉 여부 = 종가 > 시가
  4. 2번 블록 고점 대비 비율 계산
- **출력**: 몸통 범위[하단, 상단], 양봉 여부

#### 2번 블록 몸통 계산

- **입력**: 2번 블록 D일 OHLC 데이터
- **알고리즘**: 1번 블록과 동일
- **출력**: 몸통 범위[하단, 상단], 양봉 여부

#### 2번 블록 저가 추출

- **입력**: 2번 블록 D/D+1/D+2일 OHLC 데이터
- **알고리즘**:
  1. D일 저가 추출
  2. D+1일 저가 추출 (패턴에 따라)
  3. D+2일 저가 추출 (패턴에 따라)
  4. 최저가 = min(D저가, D+1저가, D+2저가)
- **출력**: 각 일자별 저가, 최저가

#### 돌파 여부 판정

- **입력**: 2번 블록 고점, 2번 블록 이후 가격 데이터
- **알고리즘**:
  1. 2번 블록 D일 이후 순차 탐색
  2. 3일 연속 종가 > 블록 고점 확인
  3. 조건 만족 시 돌파 확정일 기록
  4. 돌파까지 소요 일수 계산
  5. 시도 횟수 카운트 (고점 터치 후 실패 횟수)
- **출력**: 돌파 여부(Boolean), 돌파일, 소요 일수, 시도 횟수

#### 지지 레벨 판정

- **입력**: 지지선 가격대, 2번 블록 이후 가격 데이터
- **알고리즘**:
  1. 2번 블록 이후 최저점 탐색
  2. 최저점이 어느 지지선 범위에 속하는지 판정:
     - 2번 블록 몸통 내부 → "block_2_body"
     - 1번 블록 몸통 내부 → "block_1_body"
     - 2번 블록 저가 부근 → "block_2_lows"
     - 모든 지지선 이탈 → "broken"
  3. 지지선 침투 깊이 계산 (음수 %)
  4. 지지선 아래 체류 일수 카운트
  5. 회복 여부 확인 (3일 내 지지선 위로 복귀)
  6. 지지 강도 점수 계산 (침투 깊이 기반, 1~10점)
- **출력**: 최종 지지 레벨, 강도 점수, 테스트 기록
...
**Step 0: 지지 레벨별 기초 통계 분석** ⭐ NEW

- **목적**: 지지 레벨과 성공률/수익률의 관계 파악
- **분석 방법**:
  - **지지 레벨별 성공률 분석**
    - 2번 블록 몸통 지지 → 돌파 성공률? Level 4 비율?
    - 1번 블록 몸통 지지 → 돌파 성공률? Level 3 비율?
    - 2번 블록 저가 지지 → 돌파 성공률? Level 2 비율?
    - 모든 지지선 붕괴 → 실패율?
  - **조정 폭 분석**
    - 지지 레벨별 평균 조정폭 (블록 고점 대비 %)
    - 성공 케이스의 최대 허용 조정폭
    - 실패 케이스의 평균 붕괴 깊이
  - **시간 분석**
    - 지지 레벨별 평균 돌파 소요 일수
    - 지지 테스트 횟수와 성공률의 관계
  - **강도 점수 검증**
    - 지지 강도 점수(1~10)와 최종 수익률의 상관관계
- **탐색 질문**:
  - 2번 블록 몸통 지지가 정말 가장 강한가? (성공률 95%+?)
  - 1번 블록 몸통 이탈 시 실패 확률은?
  - 침투 깊이 -5% 이상 시 회복 가능성은?
  - 지지 테스트 3회 이상 시 성공률 변화는?
- **산출물**:
  - 지지 레벨별 성공률 표
  - 조정폭-성공률 산점도
  - 지지 강도 점수 분포 차트

#### Step 0.5: 탐색적 데이터 분석 (EDA)
...
#### Step 1: 돌파 성공 vs 실패 팩터 분석 ⭐ UPDATED

- **목적**: 돌파 성공 여부와 상관관계가 있는 팩터 발견
- **분석 방법**:
  - 전체 케이스를 돌파 성공 vs 돌파 실패로 2분류
  - Random Forest 분류 모델로 Feature Importance 추출
  - SHAP으로 돌파 성공/실패에 영향을 주는 팩터 순위화
  - 통계적 검정 (t-test, Mann-Whitney U test)으로 유의미한 차이 확인
- **핵심 질문** (우선순위 반영):
  - **[1순위] 지지 레벨이 돌파 성공의 가장 강력한 예측 변수인가?**
    - 2번 블록 몸통 지지 → 돌파 성공률 얼마?
    - 1번 블록 몸통 지지 → 돌파 성공률 얼마?
    - 모든 지지선 붕괴 → 돌파 실패율 얼마?
    - 지지 강도 점수와 돌파 성공률의 상관계수는?
  - **[2순위] 다른 팩터들의 영향력은?**
    - 신고가 등급과 돌파 성공률의 관계는? (지지 레벨 통제 후)
    - 외국인+기관 순매수가 필수인가?
    - 재무 턴어라운드가 필요한가?
    - 산업 특성이 영향을 미치는가?
  - **[3순위] 실패 원인 분석**
    - 지지선 붕괴 시 다른 팩터가 좋아도 실패하는가?
    - 지지선을 지켰는데도 실패하는 케이스의 특징은?
- **산출물**:
  - 지지 레벨별 돌파 성공률 테이블
  - Feature Importance 순위 (지지 레벨 vs 기타 팩터)
  - SHAP 값 분석 차트
