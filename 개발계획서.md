...
```python
# 계층 분리
1. Model Layer (데이터 계층)
   - DatabaseManager: PostgreSQL/SQLite 데이터 접근
   - DataCollector: API 데이터 수집
   - BlockDetector: 거래량 블록 탐지
   - MLPredictor: 머신러닝 예측

2. View Layer (UI 계층)
   - MainWindow: 메인 윈도우
   - DashboardWidget: 대시보드 화면
   - StockAnalysisWidget: 종목 분석 화면
   - ChartWidget: 차트 시각화
   - SettingsDialog: 설정 대화상자

3. Presenter Layer (컨트롤러 계층)
   - DashboardPresenter: 대시보드 로직
   - StockPresenter: 종목 분석 로직
   - ChartPresenter: 차트 업데이트 로직
   - Signals/Slots를 통한 View-Presenter 통신
```
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RoboStock - 장기투자 모델")
        ...
        self.stock_analysis_view = StockAnalysisView()
        self.block_monitor_view = BlockMonitorView()
        self.supply_view = SupplyAnalysisView()
        self.portfolio_view = PortfolioView()

        self.tabs.addTab(self.dashboard_view, "대시보드")
        self.tabs.addTab(self.stock_analysis_view, "종목 분석")
        self.tabs.addTab(self.block_monitor_view, "블록 모니터링")
        self.tabs.addTab(self.supply_view, "수급 분석")
        self.tabs.addTab(self.portfolio_view, "포트폴리오")
        ...
        self.update_timer.start(60000)  # 1분마다
...
class DashboardView(QWidget):
    ...
    def __init__(self):
        ...
        self.kospi_label = QLabel("KOSPI: 2,500 (+1.2%)")
        self.kosdaq_label = QLabel("KOSDAQ: 850 (+0.8%)")
        market_layout.addWidget(self.kospi_label)
        market_layout.addWidget(self.kosdaq_label)
        market_layout.addStretch()
        layout.addLayout(market_layout)

        # 1번 블록 발생 종목 테이블
        layout.addWidget(QLabel("🔴 오늘 발생한 1번 거래량 블록"))
        self.block1_table = QTableWidget()
        self.block1_table.setColumnCount(8)
        self.block1_table.setHorizontalHeaderLabels([
            "종목코드", "종목명", "현재가", "거래량", "거래대금",
            "전일대비", "2년래 최대", "신고가"
        ])
        layout.addWidget(self.block1_table)

        # 2번 블록 발생 종목 테이블 (투자 기회!)
        layout.addWidget(QLabel("🟢 오늘 발생한 2번 거래량 블록 (투자 기회!)"))
        self.block2_table = QTableWidget()
        self.block2_table.setColumnCount(10)
        self.block2_table.setHorizontalHeaderLabels([
            "종목코드", "종목명", "현재가", "1번블록일", "경과일",
            "필수조건", "품질점수", "예상수익률", "예상기간", "상세보기"
        ])
        layout.addWidget(self.block2_table)
...
class StockAnalysisView(QWidget):
    """종목 분석 - 개별 종목 상세 정보"""
    ...
    def __init__(self):
        ...
        info_tabs.addTab(self.create_block_history_tab(), "블록 히스토리")
        info_tabs.addTab(self.create_financial_tab(), "재무 분석")
        info_tabs.addTab(self.create_supply_tab(), "수급 분석")
        info_tabs.addTab(self.create_prediction_tab(), "ML 예측")
        splitter.addWidget(info_tabs)

        splitter.setStretchFactor(0, 6)  # 차트 60%
        splitter.setStretchFactor(1, 4)  # 정보 40%
    ...
    def search_stock(self):
        """종목 검색 및 분석"""
        stock_code = self.search_input.text()
        # Presenter를 통해 데이터 로드
        pass

    def create_block_history_tab(self):
        """블록 히스토리 탭"""
        from PySide6.QtWidgets import QTableWidget
        table = QTableWidget()
        table.setColumnCount(7)
        table.setHorizontalHeaderLabels([
            "1번블록일", "2번블록일", "경과일", "최고수익률",
            "달성기간", "MDD", "100%달성"
        ])
        return table
    ...
    def create_financial_tab(self):
        ...
        table.setHorizontalHeaderLabels([
            "연도", "영업이익(억)", "자본총계(억)", "ROE(%)", "ROA(%)"
        ])
        return table

    def create_supply_tab(self):
        """수급 분석 탭"""
        widget = SupplyChartWidget()  # 외국인/기관 누적 추세 차트
        return widget
    ...
    def create_prediction_tab(self):
        ...
        widget = QWidget()
        layout = QVBoxLayout()
        layout.addWidget(QLabel("예측 성공 확률: 75%"))
        layout.addWidget(QLabel("예상 최고 수익률: 120%"))
        layout.addWidget(QLabel("예상 달성 기간: 2.5년"))
        layout.addWidget(QLabel("클러스터: 고성공 그룹"))
...
```python
class BlockMonitorView(QWidget):
    """블록 모니터링 - 2번 블록 대기 종목"""
    ...
    def __init__(self):
        ...
        layout.addWidget(QLabel("1번 블록 발생 후 2번 블록 대기 중인 종목"))

        # 필터 옵션
        filter_layout = QHBoxLayout()
        filter_layout.addWidget(QLabel("경과일:"))
        self.days_filter = QComboBox()
        self.days_filter.addItems(["전체", "30일 이내", "60일 이내", "90일 이내"])
        ...
        self.monitor_table.setHorizontalHeaderLabels([
            "종목코드", "종목명", "1번블록일", "경과일", "현재가",
            "1번블록 대비", "거래량 추이", "2번블록 가능성", "알림설정"
        ])
...
```python
class SupplyAnalysisView(QWidget):
    """수급 분석 - 투자자별 매매 추세"""
    ...
    def __init__(self):
        ...
        # 차트
        self.supply_chart = SupplyChartWidget()
        layout.addWidget(self.supply_chart)

        # 수급 강도 순위 테이블
        layout.addWidget(QLabel("수급 강도 상위 종목"))
        self.supply_table = QTableWidget()
        self.supply_table.setColumnCount(7)
        self.supply_table.setHorizontalHeaderLabels([
            "종목명", "외국인 누적", "기관 누적", "외+기 추세",
            "매수강도", "연속매수일", "종목분석"
        ])
...
class PortfolioView(QWidget):
    ...
    def __init__(self):
        ...
        summary_layout.addWidget(QLabel("총 평가액: 10,000,000원"))
        summary_layout.addWidget(QLabel("총 수익률: +25%"))
        summary_layout.addWidget(QLabel("보유 종목 수: 5개"))
        layout.addLayout(summary_layout)

        # 보유 종목 테이블
        self.portfolio_table = QTableWidget()
        self.portfolio_table.setColumnCount(10)
        self.portfolio_table.setHorizontalHeaderLabels([
            "종목명", "매수일", "매수가", "현재가", "수익률",
            "블록상태", "고점대비", "보유기간", "매도신호", "상세보기"
        ])
...
class StockChartWidget(QWidget):
    ...
    def plot_stock(self, df: pd.DataFrame, block_dates: list):
        """
        주가 차트 그리기

        Args:
            df: OHLCV 데이터 (date, open, high, low, close, volume, ma60)
            block_dates: 블록 발생 날짜 리스트 [(date, type), ...]
        """
        self.figure.clear()

        # 2개 서브플롯: 가격 + 거래량
        ax1 = self.figure.add_subplot(211)  # 가격
        ax2 = self.figure.add_subplot(212)  # 거래량

        # 캔들스틱 (간단 버전: mplfinance 사용 권장)
        ...
        # 60일 이동평균선
        ax1.plot(df.index, df['ma60'], color='orange', linewidth=2, label='60일 이평선')

        # 블록 발생일 표시
        for date, block_type in block_dates:
            idx = df.index.get_loc(date)
            color = 'red' if block_type == '1번' else 'green'
            ax1.axvline(x=idx, color=color, linestyle='--', alpha=0.7)
            ax1.text(idx, df.loc[date, 'high'] * 1.02, block_type,
        ...
        ax1.set_title("주가 차트", fontsize=14)
        ax1.legend()
...
```python
class SupplyChartWidget(QWidget):
    """수급 추세 차트 - 외국인/기관/개인 누적 매매"""
    ...
    def plot_supply(self, df: pd.DataFrame):
        """
        수급 차트 그리기

        Args:
            df: 수급 데이터 (date, foreign_net, institution_net,
                            individual_net, foreign_cumsum, institution_cumsum)
        """
        self.figure.clear()

        # 2개 서브플롯: 일별 순매수 + 누적 순매수
        ax1 = self.figure.add_subplot(211)
        ax2 = self.figure.add_subplot(212)

        # 일별 순매수
        ax1.plot(df['date'], df['foreign_net'], label='외국인', color='red')
        ax1.plot(df['date'], df['institution_net'], label='기관', color='blue')
        ax1.plot(df['date'], df['individual_net'], label='개인', color='green')
        ax1.axhline(y=0, color='black', linestyle='-', linewidth=0.5)
        ax1.set_title("일별 순매수", fontsize=12)
        ax1.legend()
        ax1.grid(True, alpha=0.3)

        # 누적 순매수 + 추세선
        ax2.plot(df['date'], df['foreign_cumsum'], label='외국인 누적', color='red')
        ax2.plot(df['date'], df['institution_cumsum'], label='기관 누적', color='blue')

        # 선형회귀 추세선
        from scipy.stats import linregress
        x = range(len(df))
        slope_f, intercept_f, _, _, _ = linregress(x, df['foreign_cumsum'])
        slope_i, intercept_i, _, _, _ = linregress(x, df['institution_cumsum'])
        ax2.plot(df['date'], [slope_f * i + intercept_f for i in x],
                '--', color='red', alpha=0.5, label='외국인 추세')
        ax2.plot(df['date'], [slope_i * i + intercept_i for i in x],
                '--', color='blue', alpha=0.5, label='기관 추세')

        ax2.set_title("누적 순매수 (추세선)", fontsize=12)
        ax2.legend()
        ax2.grid(True, alpha=0.3)
...
class DashboardPresenter(QObject):
    """대시보드 Presenter - 비즈니스 로직"""

    # Signals (View 업데이트용)
    block1_data_updated = Signal(list)  # 1번 블록 데이터
    block2_data_updated = Signal(list)  # 2번 블록 데이터
    market_data_updated = Signal(dict)  # 시장 지수

    def __init__(self, database_manager, block_detector):
        super().__init__()
        self.db = database_manager
        self.detector = block_detector

    def load_today_blocks(self):
        """오늘 발생한 블록 로드"""
        from datetime import date
        today = date.today()

        # 1번 블록
        block1_list = self.detector.detect_block1_today(today)
        self.block1_data_updated.emit(block1_list)

        # 2번 블록
        block2_list = self.detector.detect_block2_today(today)
        self.block2_data_updated.emit(block2_list)

    def load_market_indices(self):
        """시장 지수 로드"""
        kospi = self.db.get_index_data('KOSPI')
        kosdaq = self.db.get_index_data('KOSDAQ')
...
class DashboardView(QWidget):
    def __init__(self, presenter: DashboardPresenter):
        ...
        # Signal 연결
        self.presenter.block1_data_updated.connect(self.update_block1_table)
        self.presenter.block2_data_updated.connect(self.update_block2_table)
        self.presenter.market_data_updated.connect(self.update_market_labels)

        # 초기 로드
        self.presenter.load_today_blocks()
        self.presenter.load_market_indices()

    def update_block1_table(self, data: list):
        """1번 블록 테이블 업데이트"""
        self.block1_table.setRowCount(len(data))
        for i, item in enumerate(data):
            self.block1_table.setItem(i, 0, QTableWidgetItem(item['code']))
            self.block1_table.setItem(i, 1, QTableWidgetItem(item['name']))
            # ... 나머지 컬럼

    def update_block2_table(self, data: list):
        """2번 블록 테이블 업데이트"""
        # 구현
        pass

    def update_market_labels(self, data: dict):
        """시장 지수 레이블 업데이트"""
        kospi = data['kospi']
        self.kospi_label.setText(f"KOSPI: {kospi['close']:,.0f} ({kospi['change']:+.1f}%)")
```
...
**다크 테마 적용**
```python
# ui/resources/styles/dark_theme.qss
QMainWindow {
...
def main():
    ...
    # 다크 테마 로드
    with open('ui/resources/styles/dark_theme.qss', 'r', encoding='utf-8') as f:
...
class AlertManager:
    ...
    def __init__(self, main_window):
        ...
        # 시스템 트레이 아이콘
        self.tray_icon = QSystemTrayIcon(QIcon("ui/resources/icons/app_icon.png"))
        self.tray_icon.setToolTip("RoboStock")
        self.tray_icon.activated.connect(self.on_tray_activated)
        self.tray_icon.show()

    def show_block2_alert(self, stock_code, stock_name):
        """2번 블록 발생 알림"""
        self.tray_icon.showMessage(
            "🟢 2번 블록 발생!",
            f"{stock_name}({stock_code})에서 2번 거래량 블록이 발생했습니다.\n투자 기회를 확인하세요!",
            QSystemTrayIcon.Information,
            5000  # 5초
        )

    def show_profit_alert(self, stock_name, profit_pct):
        """목표 수익률 달성 알림"""
        self.tray_icon.showMessage(
            "🎉 목표 달성!",
            f"{stock_name}이(가) {profit_pct:.1f}% 수익을 달성했습니다!",
            QSystemTrayIcon.Information,
            5000
        )
...
class SettingsDialog(QDialog):
    ...
    def __init__(self):
        ...
        self.update_interval.setRange(30, 600)  # 30초 ~ 10분
        self.update_interval.setValue(60)
        self.update_interval.setSuffix(" 초")
        layout.addRow("데이터 업데이트 주기:", self.update_interval)

        # 알림 설정
        self.enable_block2_alert = QCheckBox("2번 블록 발생 시 알림")
        self.enable_block2_alert.setChecked(True)
        layout.addRow("", self.enable_block2_alert)

        self.enable_profit_alert = QCheckBox("목표 수익률 달성 시 알림")
        self.enable_profit_alert.setChecked(True)
        layout.addRow("", self.enable_profit_alert)

        # 필터 기본값
        self.default_trading_value = QSpinBox()
        self.default_trading_value.setRange(100, 10000)
        self.default_trading_value.setValue(500)
        self.default_trading_value.setSuffix(" 억원")
        layout.addRow("최소 거래대금:", self.default_trading_value)
